...
* Copy the executable anywhere you want
    * Linux:  MeshStat
    * Windows:  MeshStat.exe

* Create a MeshStat.ini file in the same directory.  See the sample
  (commented out) version in the file Sample-MeshStat.ini for
  examples of what can be set.

    * Make sure the 'period' is set to a number greater than the
      number of nodes!
...
- Each node should display its status in 4 lines, if you do not see
  the node name on the first line, it is likely you may need to
  adjust 'pane_width_chars' and 'pane_interline_space' to get it all
  on 4 lines.  There is a brief explanation of these parameters in
  the file 'Sample-MeshStat.ini'.  You may want to work this out
  with a smaller set of nodes before using a larger list of nodes.

- WARNING: With more than a few nodes, avoid trying to move the
  windows while the nodes are being updated.  It may not be
  responsive and lead to problems that may require restarting.
